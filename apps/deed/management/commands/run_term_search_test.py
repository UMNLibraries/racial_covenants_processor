...
class Command(BaseCommand):
    ...
    def get_test_deedpages(self, workflow=None, n=100):
        ...
        if workflow:
            random_deedpages = DeedPage.objects.filter(workflow=workflow).order_by('?').values('workflow__slug', 's3_lookup', 'page_ocr_json', 'page_ocr_text', 'page_image_web', 'page_stats', 'public_uuid')[:n]
        else:
    ...
    def handle(self, *args, **kwargs):
        ...
        self.term_test_result_path = os.path.join(
            settings.BASE_DIR, 'data', 'main_exports', f"{workflow_slug}_fuzzy_term_search_test_{now}.csv")
        ...
        random_deedpages = self.get_test_deedpages(workflow, num_results)
