...
class CovenantsParcelManager(models.Manager):
    ...
    def get_queryset(self):
        from apps.zoon.models import ManualCovenant, ZooniverseSubject
        ...
        return (
            ...
            .filter(bool_covenant=True)
            .annotate(
                cov_type=Case(
                    When(Exists(oldest_deed), then=Value("zooniverse")),
                    When(Exists(oldest_deed_manual), then=Value("manual")),
                    default=Value(""),
                    output_field=CharField(),
                )
            )
            .annotate(add_mod=F("plat_name"))
            .annotate(block_mod=F("block"))
            .annotate(lot_mod=F("lot"))
            .annotate(street_add=F("street_address"))
            .annotate(ph_dsc_mod=F("phys_description"))
            .annotate(cnty_name=F("county_name"))
            .annotate(cnty_fips=F("county_fips"))
            .annotate(cnty_pin=F("pin_primary"))
            .annotate(
                deed_date=Case(
                    When(
                        Exists(oldest_deed),
            ...
            .annotate(
                deed_page_1=Case(
                    When(
                        Exists(oldest_deed),
                        then=Subquery(
                            oldest_deed.values("subject_1st_page__s3_lookup")
                        ),
                    ),
                    default=Value(""),
                    output_field=CharField(),
            ...
            .annotate(
                deed_page_2=Case(
                    When(
                        Exists(oldest_deed),
                        then=Subquery(
                            oldest_deed.values("subject_2nd_page__s3_lookup")
                        ),
                    ),
                    default=Value(""),
                    output_field=CharField(),
            ...
            .annotate(
                deed_page_3=Case(
                    When(
                        Exists(oldest_deed),
                        then=Subquery(
                            oldest_deed.values("subject_3rd_page__s3_lookup")
                        ),
                    ),
                    default=Value(""),
                    output_field=CharField(),
            ...
            .annotate(
                manual_cx=Case(
                    ...
                    When(Exists(oldest_deed_manual), then=Value(True)),
                    default=Value(False),
...
class Parcel(models.Model):
    ...
    state = models.CharField(
        db_index=True, max_length=2, null=True, blank=True, choices=US_STATES
    )
    ...
    plat_standardized = models.CharField(
    ...
    subdivision_spatial = models.ForeignKey(
        Subdivision, on_delete=models.SET_NULL, null=True
    )
    ...
    class Meta:
        indexes = [
            models.Index(
                fields=["subdivision_spatial_id", "workflow_id"],
                name="workflow_and_subdivision_index",
            ),
            models.Index(
                fields=["workflow_id", "bool_covenant"],
                name="workflow_and_bool_cov_index",
            ),
        ]
...
class ParcelJoinCandidate(models.Model):
    ...
    plat_name_standardized = models.CharField(max_length=255, db_index=True, null=True)
    """Addition/plat name after filtering"""
    join_string = models.CharField(max_length=500, db_index=True, null=True)
...
class PMTilesExport(models.Model):
...
class ParcelPMTilesExport(models.Model):
    """A PMTiles export of every parcel in a workflow, used as the selectable layer in the map editor. Generated by dump_covenants_shapefile with --all-parcels --pmtiles."""
...
class CovenantDrawnGeometry(models.Model):
    """A shape drawn by hand in the map editor for a covenant that does not map
    cleanly onto modern parcels -- a partial lot, a strip, a vacated street.

    Kept alongside parcel matches rather than replacing them: a covenant can
    cover several parcels plus an area no parcel describes. Each drawn shape
    materializes its own flat CovenantedParcel row, with no parcel FK, so it
    flows into every export and the baked tiles like any other covenant."""
    workflow = models.ForeignKey(
         "zoon.ZooniverseWorkflow", null=True, on_delete=models.SET_NULL)
    zooniverse_subject = models.ForeignKey(
        "zoon.ZooniverseSubject", null=True, blank=True, on_delete=models.CASCADE)
    manual_covenant = models.ForeignKey(
        "zoon.ManualCovenant", null=True, blank=True, on_delete=models.CASCADE)

    geom_4326 = models.MultiPolygonField(srid=4326)
    """Multipolygon geometry of the drawn shape, in SRID:4326 (WGS-84)."""

    comments = models.TextField(null=True, blank=True)
    """Why this shape was drawn rather than parcels being selected."""

    added_by = models.CharField(max_length=255, null=True, blank=True)
    """Username of whoever drew it, for provenance."""
    ...
    class Meta:
        ordering = ('-id',)
        indexes = [
            models.Index(fields=['workflow', 'zooniverse_subject']),
            models.Index(fields=['workflow', 'manual_covenant']),
        ]

    def __str__(self):
        return f"Drawn geometry {self.pk} for {self.covenant}"

    @property
    def covenant(self):
        return self.zooniverse_subject or self.manual_covenant

    def save(self, *args, **kwargs):
        if self.workflow_id is None and self.covenant is not None:
            self.workflow = self.covenant.workflow
        super().save(*args, **kwargs)


@receiver(models.signals.post_save, sender=CovenantDrawnGeometry)
def drawn_geometry_post_save(sender, instance=None, **kwargs):
    '''Rebuild this shape's flat covenant row and re-bake the tiles, the same
    way saving a covenant does for its parcel matches.'''
    if instance is None:
        return
    from apps.parcel.utils.export_utils import save_flat_drawn_covenant
    save_flat_drawn_covenant(instance)


@receiver(models.signals.post_delete, sender=CovenantDrawnGeometry)
def drawn_geometry_post_delete(sender, instance=None, **kwargs):
    '''The flat row goes with it by CASCADE, but the tiles still need re-baking
    so the shape stops appearing on the map.'''
    if instance is None or instance.workflow is None:
        return
    from apps.parcel.utils.pmtiles_utils import trigger_pmtiles_export
    trigger_pmtiles_export(instance.workflow)


class ParcelSearchIndex(models.Model):
    """A gzipped JSON index of every parcel in a workflow -- join strings, PIN,
    address, plat/block/lot and a centroid -- loaded by the map editor so
    parcels can be searched in the browser. Baked PMTiles can only be queried
    for what is already on screen and carry no join strings, hence this sidecar.
    Generated by dump_covenants_shapefile with --parcel-search-index."""
    workflow = models.ForeignKey(
         "zoon.ZooniverseWorkflow", null=True, on_delete=models.SET_NULL)
...
class DischargeCSVExport(models.Model):
    """A CSV export of all documents that have confirmed racial covenants, whether they have been mapped or not. Note that the count of these rows will differ from the main Deed Machine exports because this is a list of all documents, not all parcels with covenants. (Racial covenants on a propery are often repeated in subsequent sales.) This export is useful for determining which documents may need to be flagged in property records for racial language. Generated by dump_all_covenanted_docs management command. File saved to S3 by default, but can be saved locally as well."""
...
class CovenantedParcel(models.Model):
    ...
    parcel = models.ForeignKey("parcel.Parcel", null=True, on_delete=models.SET_NULL)
    drawn_geometry = models.ForeignKey(
        "parcel.CovenantDrawnGeometry", null=True, blank=True, on_delete=models.CASCADE
    )
    '''Set instead of parcel when this row came from a shape drawn by hand in
    the map editor. CASCADE so deleting the drawing removes its export row --
    delete_flat_covenanted_parcels() only knows how to clear parcel-derived
    rows.'''
    ...
    main_image = models.CharField(max_length=501, null=True)
    ...
    city = models.CharField(max_length=502, null=True)
    ...
    add_mod = models.CharField(max_length=1200, null=True)
    block_mod = models.CharField(max_length=505, null=True)
    lot_mod = models.CharField(max_length=506, null=True)
