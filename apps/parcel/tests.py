...
from apps.parcel.models import (
    CovenantedParcel,
    ManualParcelCandidate,
    Parcel,
    PMTilesExport,
)
from apps.parcel.utils import export_utils
from apps.parcel.utils.export_utils import (
    EXPORT_FIELDS_ORDERED,
    delete_flat_covenanted_parcels,
    save_flat_covenanted_parcels,
)
from apps.parcel.utils.parcel_utils import (
    get_blocks,
    get_lots,
    standardize_addition,
    write_join_strings,
)
from apps.parcel.utils.pmtiles_utils import trigger_pmtiles_export
from apps.zoon.models import ManualCovenant, ZooniverseSubject, ZooniverseWorkflow
...
class JoinStringTests(TestCase):
    ...
    def test_standardize_addition_weird_1st(self):
        ...
        self.assertEqual(
            standardize_addition("1st Addition To Eskesens Lots"),
            "1st to eskesens lots",
        )
        self.assertEqual(
            standardize_addition("2nd Addition To Eskesens Lots"),
            "2nd to eskesens lots",
        )
        self.assertEqual(
            standardize_addition("1st Addition To Maple Ridge Estates"),
            "1st to maple ridge estates",
        )
        self.assertEqual(
            standardize_addition("3rd Addition To Maple Ridge Estates"),
            "3rd to maple ridge estates",
        )
        self.assertEqual(
            standardize_addition("Fourth Addition To Maple Ridge Estates"),
            "4th to maple ridge estates",
        )
        self.assertEqual(
            standardize_addition("First Addition To Maple Ridge Estates"),
            "1st to maple ridge estates",
        )
        self.assertEqual(standardize_addition("1st Addition to Jane's"), "1st to janes")
        self.assertEqual(
            standardize_addition("10th Addition To Breezy Point Estates"),
            "10th to breezy point estates",
        )
    ...
    def test_standardize_addition_negatives(self):
        ...
        self.assertEqual(
            standardize_addition("Jane's Add It Up"), "janes add it up"
        )  # Not sure I want this, may reconsider

    def test_standardize_addition_subdivision(self):
        for example in ["Another Subdivision", "Another SUBD.", "Another SUB"]:
    ...
    def test_write_join_strings_with_no_2(self):
        ...
        lot = "1"
        self.assertEqual(
            write_join_strings(addition, block, lot)[0]["join_string"],
            "arden hills 2 block 7 lot 1",
        )
    ...
    def test_write_join_strings_multi(self):
        ...
        self.assertEqual(join_strings, "janes block 1 lot 1; janes block 1 lot 2")
...
class ParcelCandidateTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        management.call_command(
            "rebuild_parcel_spatial_lookups", workflow=workflow.workflow_name
        )
    ...
    def test_manualparcelcandidate_on_rebuild(self):
        ...
        self.assertEqual(
            parcel.parceljoincandidate_set.filter(
                join_string="mpc base block 1 lot 6"
            ).count(),
            1,
        )
...
class CovenantedParcelTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        management.call_command(
            "rebuild_covenant_spatial_lookups", workflow=workflow.workflow_name
        )
        management.call_command(
            "match_parcels", "--test", workflow=workflow.workflow_name
        )
    ...
    def test_save_flat_covenanted_parcels(self):
        ...
        parcels = Parcel.objects.filter(
            pin_primary__in=["covenanted-parcel-1", "covenanted-parcel-2"]
        )

        # Delete covs created by match_parcels
        cleared_parcels = delete_flat_covenanted_parcels(parcels)
        self.assertEqual(
            CovenantedParcel.objects.filter(
                parcel__pin_primary="covenanted-parcel-1"
            ).count(),
            0,
        )
        self.assertEqual(
            CovenantedParcel.objects.filter(
                parcel__pin_primary="covenanted-parcel-2"
            ).count(),
            0,
        )
    ...
    def test_addition_wide_covenanted_parcel_creation(self):
        ...
        awc2 = ZooniverseSubject.objects.get(
            addition="Not Covenanted Parcel AW Addition"
        )
        awc2.save()
...
class ExportFieldsFixtureTests(TestCase):
    """fields.json is copied into mp-covenants-pmtiles-export, whose SQL builds
    the same property list. If this drifts, the Lambda silently bakes tiles with
    the wrong attributes."""

    def test_fields_json_matches_export_fields_ordered(self):
        fields_path = os.path.join(
            os.path.dirname(export_utils.__file__), "fields.json"
        )
        with open(fields_path) as f:
            fixture_fields = json.load(f)

        self.assertEqual(fixture_fields, EXPORT_FIELDS_ORDERED)


class TriggerPMTilesExportTests(TestCase):
    fixtures = ["plat", "zoon", "parcel"]

    @override_settings(PMTILES_LAMBDA_NAME="")
    def test_blank_lambda_name_is_a_noop(self):
        """Guards tests and local dev from reaching out to AWS."""
        workflow = ZooniverseWorkflow.objects.first()
        with mock.patch("boto3.client") as mock_client:
            self.assertIsNone(trigger_pmtiles_export(workflow))
        mock_client.assert_not_called()

    @override_settings(PMTILES_LAMBDA_NAME="test-pmtiles-lambda")
    def test_invokes_lambda_asynchronously(self):
        workflow = ZooniverseWorkflow.objects.first()
        with mock.patch("boto3.client") as mock_client:
            request_id = trigger_pmtiles_export(workflow)

        mock_client.return_value.invoke.assert_called_once()
        kwargs = mock_client.return_value.invoke.call_args.kwargs
        self.assertEqual(kwargs["FunctionName"], "test-pmtiles-lambda")
        self.assertEqual(kwargs["InvocationType"], "Event")

        payload = json.loads(kwargs["Payload"])
        self.assertEqual(payload["workflow_id"], workflow.pk)
        self.assertEqual(payload["workflow_slug"], workflow.slug)
        self.assertEqual(payload["request_id"], request_id)


@override_settings(PMTILES_CALLBACK_TOKEN="secret")
class PMTilesCallbackViewTests(TestCase):
    fixtures = ["plat", "zoon"]

    def setUp(self):
        self.url = reverse("pmtiles_callback")
        self.workflow = ZooniverseWorkflow.objects.first()
        self.payload = {
            "workflow_id": self.workflow.pk,
            "pmtiles_key": "main_exports/mn-test-county_covenants_20260710_1200.pmtiles",
            "covenant_count": 42,
        }

    def post(self, payload=None, token="secret"):
    ...
    def test_creates_pmtiles_export(self):
        response = self.post()
        self.assertEqual(response.status_code, 201)

        export = PMTilesExport.objects.get(pk=response.json()["id"])
        self.assertEqual(export.workflow, self.workflow)
        self.assertEqual(export.covenant_count, 42)
        self.assertEqual(export.pmtiles.name, self.payload["pmtiles_key"])

    def test_repeat_callback_is_idempotent(self):
        """AWS retries a failed async invoke up to twice, on the same key."""
        self.assertEqual(self.post().status_code, 201)
        second = self.post()
        self.assertEqual(second.status_code, 200)
        self.assertFalse(second.json()["created"])
        self.assertEqual(PMTilesExport.objects.count(), 1)

    def test_bad_token_is_forbidden(self):
        self.assertEqual(self.post(token="wrong").status_code, 403)
        self.assertEqual(self.post(token=None).status_code, 403)
        self.assertEqual(PMTilesExport.objects.count(), 0)

    @override_settings(PMTILES_CALLBACK_TOKEN="")
    def test_unset_token_never_fails_open(self):
        self.assertEqual(self.post(token="").status_code, 403)
        self.assertEqual(self.post(token="anything").status_code, 403)
        self.assertEqual(PMTilesExport.objects.count(), 0)

    def test_rejects_key_outside_main_exports(self):
        for bad_key in [
            "private/secrets.pmtiles",
            "main_exports/../private/secrets.pmtiles",
            "main_exports/covenants.geojson",
            "",
        ]:
            payload = {**self.payload, "pmtiles_key": bad_key}
            self.assertEqual(self.post(payload).status_code, 400, msg=bad_key)
        self.assertEqual(PMTilesExport.objects.count(), 0)

    def test_rejects_missing_fields(self):
        payload = {"pmtiles_key": self.payload["pmtiles_key"]}
        self.assertEqual(self.post(payload).status_code, 400)

    def test_unknown_workflow_is_404(self):
        payload = {**self.payload, "workflow_id": 999999}
