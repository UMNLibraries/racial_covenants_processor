...
The OpenSearch API endpoint will be available at `http://127.0.0.1:9200/<http://127.0.0.1:9200/>`__. (``DISABLE_SECURITY_PLUGIN`` keeps local setup simple by using plain HTTP with no certs or authentication; in production, we'll use Amazon OpenSearch Service with a more locked-down config.)
